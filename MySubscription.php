...
<html lang="">
    ...
    <body>
        ...
        <script type="text/javascript">
						...
						function unFollowUser_refresh(userId, bizId) {
							if(confirm('Are you sure, want to delete this Subscribed Id?')){
								$('#fancybox-loading').css('display','block');
                            $.post("unFollowUser.php", {userId: userId, bizId: bizId,subs_subscribe:'Y'})
                                    .done(function(data) {
								...
								if(data){
									alert('Successfully Deleted.');
											document.location.href = 'MySubscription.php';
									//$('div#subscribed_box_'+bizId).remove();
								}
				...
				function delete_droup(grid){
					if(confirm('Are you sure, want to delete this group?')){
						//alert(grid);
						$('#fancybox-loading').css('display','block');
						$.post("unFollowUser.php", {groupid: grid, delete_group :'Y'})
                                    .done(function(data) {
										$('#fancybox-loading').css('display','none');
										//alert(data);
										if(data){
                ...
                <section class="widget small" style="width:100%;">
                    ...
                    <div class="content">
                        ...
                        if(!empty($_REQUEST['groupid'])){
							$res = $businessManager->getTopUsers_new($_SESSION['userId'],$_REQUEST['groupid']);
						...
						}else if(!empty($_REQUEST['indivisualid'])){
							$res = $businessManager->getTopUsers_new_indivisual($_SESSION['userId'],$_REQUEST['indivisualid']);
						...
						//echo $businessManager->getTopUsers($_SESSION['userId']);
							...
							$res1 = $businessManager->getAllFolloiwingUsersAndGroups($_SESSION['userId']);
							//echo 'hi';
							//var_dump($res);
							if($res1 == ''){
								echo '<strong>No Subscription Group found.</strong>';
							}else{
								?>
                                <div class="subscribed_box" style="float:left;width:100%;">
                                    <div class="picture_class" style=" width:20%;">Picture</div>
                                    <div class="name_class" style="width:15%; text-align:centre;">Name</div>
                                    <div class="username_class" >@NewsID</div>
                                    <div class="groupname_class" >Subscribers</div>
                                    <div class="subscribe_class_bold" style="">View</div>
                                    <div class="name_class_bold" >Delete</div>
                                </div>
                                <?php
								//echo $res;
								echo $res1;
