...
class UserManager {
    ...
    function validateUser($username, $password) {
		...
		$encyPwd = $this->encryptPassword($password);
		$q = "SELECT * FROM USER_LOGIN_INFO INNER JOIN USER_INFO ON USER_LOGIN_INFO.USER_ID = USER_INFO.USER_ID WHERE USERNAME ='$username' AND PASSWORD='$encyPwd'";
		...
		if(mysql_num_rows($result)){
			...
			//print_r($Row);die;
			if($Row['IS_ACTIVE'] != 'A'){
				$_SESSION['login_faild_msg'] = 'Account is deactivated please check your mail for account activation link';
				return false;
			}
	    $ramConnection->hset('USER_LOGIN', $username, $encyPwd);
            $ramConnection->hset('USER_EMAIL_ID', $Row['EMAIL'], $Row['USER_ID']);
            $ramConnection->hset('USER_ID_EMAIL', $Row['USER_ID'], $email);
            $ramConnection->sadd('PUBLIC_PROFILES', $Row['USER_ID']);
            $ramConnection->sadd('PUBLIC_TWEETS', $Row['USER_ID']);
            $ramConnection->hset('USERNAME_ID', $Row['USERNAME'], $Row['USER_ID']);
            $ramConnection->hset('USER_ID_NAME', $Row['USER_ID'], $Row['USERNAME']);
		}
		else{
			$_SESSION['login_faild_msg'] = 'Either username or password was wrong.';
			return false;
		}
    ...
    function userNameValidation($userName) {
        try {
            $ramConnection = getRAMConnection();
	    $res = $ramConnection->hexists('USERNAME_ID', $userName);
	    if(!$res){
		getDBConnection();
		$q = "SELECT LOGIN_ID FROM USER_LOGIN_INFO WHERE USERNAME = '$userName'";
		$r = mysql_query($q);
		if(!mysql_num_rows($r))
		    return false;
		else
    ...
    function saveUser($email, $userName, $password,$fname ,$lname) {
	    ...
	    $ramConnection->hset('STATUS', $userId, 'I');
            $date = date('Y-m-d H:i:s');
            $userLoginInfoquery = "INSERT INTO USER_LOGIN_INFO (USER_ID,USERNAME,PASSWORD,CREATED_DATE_TIME,CREATED_BY,UPDATED_DATE_TIME,UPDATED_BY,IS_ACTIVE,LAST_LOGGED_IN_TIME) values ('$userId','$userName','$encryptedPassword','$date','$userId','$date','$userId','I','$date')";
            mysql_query($userLoginInfoquery) or die("Database error:<br />" . mysql_error());
			$token = md5(microtime());
			$created = date('Y-m-d H:i:s');
			$tokenQuery = "INSERT INTO TOKEN (user_id,token,type,created) values('$userId','$token','email','$created')";
			mysql_query($tokenQuery) or die("Database error:<br />" . mysql_error());
			$msg = "Hello $fname,<br/><h2>Thank you for registration</h2><br/><p>Please  Click <a href='".SITE_URL."activateAccount.php?code=".$token."'>here</a> to activate your account</p>";
			$this->sendMail($email,'Account Activation Mail',$msg);
    ...
    function getAllFolloiwingUsersAndGroups($userId) {
        ...
        //***
        if($data=='' || $data==NULL){
            $data='Subscribed NewsID and created groups are shown here.';
        }
            return $data;
    }
	...
	function validateToken($Token =''){
        getDBConnection();
        $result = mysql_query("SELECT * FROM TOKEN WHERE TOKEN = '$Token'");
		$hasRecord = mysql_num_rows($result);
        if($hasRecord > 0){
			$Row = mysql_fetch_assoc($result);
			mysql_query("UPDATE USER_LOGIN_INFO SET IS_ACTIVE = 'A' WHERE USER_ID=".$Row['user_id']);
