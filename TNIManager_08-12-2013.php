...
class TNIManager {
    ...
    function getNewsletsUI($userId, $session_id) {
        ...
        foreach ($jsonArray as $k) {
			...
			<span class="icon"><img src="' . $file_loc . '" /></span>';
                ...
                $creationDate = $k->newsletCreationDate;
                ...
                $inter .= ' <span style="width: 100px; margin-left: 15px; margin-right: 15px;"> <a id="remark_lnk_' . $k->getNewsletId() . '" href="javascript:;" data-id="' . $k->getNewsletId() . '" class="btn_remarks" > Remarks  </a></span>';
                ...
                if ($k->getCoolCount() > 0)
                    $inter .= '<span style="width: 100px; margin-left: 15px; margin-right: 15px;" class="cools" id="' . $k->getNewsletId() . 'msgId">' . $k->getCoolCount() . ' Stars </span>';
                else
                    $inter .= '<span style="width: 100px; margin-left: 15px; margin-right: 15px;" class="cools" id="' . $k->getNewsletId() . 'msgId"></span>';
                ...
                if ($remark > 0)
                    ...
                    $inter .= '<span style="width: 100px; margin-left: 15px; margin-right: 15px;" class="cools" id="' . $k->getNewsletId() . 'msgId"></span>';
                //if($_SESSION['userId']!=$k->userId){
                $share = $this->get_shares_count($k->getNewsletId());
                if ($share > 0)
                    $inter .= '<span style="width: 100px; margin-left: 15px; margin-right: 15px;" class="cools" id="' . $k->getNewsletId() . 'msgIdshares">' . $share . ' Shares  </span>';
                else
                    $inter .= '<span style="width: 100px; margin-left: 15px; margin-right: 15px;" class="cools" id="' . $k->getNewsletId() . 'msgIdshares"></span>';
                //}
                //$inter .= '<div>';
                $inter .= '</p>
			</div>';
    ...
    function getNewsletsUIByBizId($userId, $session_id, $filter) {
        ...
        if (sizeof($jsonArray) > 0) {
			...
			//$inter .= '<div>';
				...
				<a href="javascript:unCoolMessage(\'' . $k->getNewsletId() . '\');" id="' . $k->getNewsletId() . 'coolBtn" >Unstar</a>
                    ...
                    $inter .= ' <span style="width: 100px; margin-left: 10px; margin-right: 10px;"> <a id="remark_lnk_' . $k->getNewsletId() . '" href="javascript:;" data-id="' . $k->getNewsletId() . '" class="btn_remarks"> Remarks  </a></span>';
                    ...
                    if ($k->getCoolCount() > 0)
                        $inter .= '<span style="width: 100px; margin-left: 10px; margin-right: 10px;" class="cools" id="' . $k->getNewsletId() . 'msgId">' . $k->getCoolCount() . ' Stars </span>';
                    else
                        $inter .= '<span style="width: 100px; margin-left: 10px; margin-right: 10px;" class="cools" id="' . $k->getNewsletId() . 'msgId"></span>';
                    //$msgManager = new MessageManager;
                    //$iidd = str_replace('i', '', $_GET['id']);
                    //$commentsMap = $msgManager->getCommentsMap($iidd , $_SESSION['userId']);
                    $remark = $this->get_remarks_count($k->getNewsletId());
        ...
        if (strlen($inter) == 0) {
            //***
            $inter = "Search a NewsID, subscribe to it and get instant feeds here.";
        }
        return $inter;
    }
			...
			//$inter .= '<div>';
                    ...
                    if ($_SESSION['userId'] != $k->userId) {
                        if ($k->shareCount > 0)
                            ...
                            $inter .= '<span style="width: 100px; margin-left: 10px; margin-right: 10px;" class="cools" id="' . $k->getNewsletId() . 'msgIdshares"></span>';
                    }
                    //$inter .= '<div>';
                    $inter .= '</p>
			</div>';
    ...
    static function getNewsLetIdsByBizId($userId, $session_id) {
        ...
        try {
            ...
            $msgDate = $ramConnection->hmget('MSG_CREATION_DATE', $data);
            ...
            foreach ($msgInfo as $k => $v) {
                ...
                $tni->setNewsletCreationDate($msgDate[$k]);
