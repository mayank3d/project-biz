	...
	function getAllFolloiwingUsersAndGroups($userId) {
        ...
        if ($num_in>0) {
			...
			if (file_exists($path_validate_in)){
              ...
              //  $count++;
				...
				<div style="float: left; text-align: left; width: 30%; margin: 0px;">'.$name['FIRST_NAME'].' '.$name['LAST_NAME'].'<br>
				@'.$name['USERNAME'].'
				</div>
        ...
        if ($num>0) {
			...
			if (file_exists($path_validate)){
				...
				<div style="float: left; text-align: left; width: 30%; margin: 0px;">'.$row['GROUP_NAME'].'<br>@Group</div>';
